...
void *download_thread(void *cb_threaddata)
{
	...
	int stop_bx = threaddata->stop_bx;
		...
		for (int neg = -1; neg <= 1; neg += 2)
		{
			...
			do
			{
				...
				for (int i = 0; i < 32; i++)
				{
					...
					c.bx = current_bx;
					c.sub_bx = 0;
					capture_bx.push_back(c);
				}

				if (!card->setInputLinkCaptureBX(neg < 0, capture_bx ))
				{
					printf("Error setInputLinkCaptureBX for phi=%d\n", threaddata->phi );
					threaddata->error = true;
					delete card;
					return NULL;
				}
...
int main(int argc, char *argv[])
{
	if (argc != 4)
	{
		printf("input_capture output_dir start_bx stop_bx\n");
		return 1;
	}

	std::istringstream ss_start_bx (argv[2]);

	uint32_t start_bx, stop_bx;
	...
	std::istringstream ss_stop_bx (argv[3]);

	if (!(ss_stop_bx >> stop_bx))
	{
		std::cout << "Invalid alignSubBX number" << argv[3] << '\n';
		exit(1);
	}
